...
export default {
  ...
  setup() {
    ...
    const exportall = async () => {
      ...
      const loadingInstance = ElLoading.service({ fullscreen: true,text:loadingText })

      let recordList = [];
      let hasMorePage = false;
      let nextPageToken = undefined;
      do {
        const { hasMore, pageToken, records } = await table.getRecordListByPage(
          {
        ...
        recordList = recordList.concat(
          // 插件作者仅用到了records的id字段，所以这里只返回id字段
          records.recordIdList.map((e) => {
            return {
              id: e,
            };
          })
        ...
        for (const record of recordList) {
          const val = await field.getValue(record.id);
            ...
            if(val){
              ...
              num++
              loadingText.value =  i18n.global.t('progress', {num})
          }
        }
      }
...
<template>
    <el-form ref="form" class="form" :model="formData" label-position="top" v-loading="loading">
